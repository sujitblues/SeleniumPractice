...
System.out.println(flag);
		/*
		 * WebElement countryEle=driver.findElement(country); Select select=new
		 * Select(countryEle); List<WebElement>countryOptions=select.getOptions();
		 * System.out.println("total values :" +countryOptions.size());
		 * 
		 * for(WebElement e:countryOptions) { String text=e.getText();
		 * System.out.println(text); } int actCount=getDropDownValueCount(country);
		 * if(actCount==233) { System.out.println("Pass--Count"); } List<String>
		 * actCountryList=getAllDropDownOptions(country);
